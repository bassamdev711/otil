...
class AppController extends ChangeNotifier {
  ...
  BookingRecord? activeBookingForRoom(String roomId) {
    for (final booking in bookings) {
      if (booking.roomId == roomId && (booking.status == 'checkedIn' || booking.status == 'pending')) return booking;
    }
    return null;
  }
...
class _RoomsPageState extends ConsumerState<RoomsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = c.rooms.where((room) {
      final matchesSearch = q.isEmpty || room.number.toLowerCase().contains(q) || room.type.toLowerCase().contains(q) || (c.guestById(c.activeBookingForRoom(room.id)?.guestId ?? '')?.name.toLowerCase().contains(q) ?? false);
      final matchesStatus = statusFilter == 'الكل' || room.status == statusFilter;
      final matchesType = typeFilter == 'الكل' || room.type == typeFilter;
      final matchesFloor = floorFilter == 'الكل' || room.floor.toString() == floorFilter;
      return matchesSearch && matchesStatus && matchesType && matchesFloor;
    }).toList();
    return ListView(padding: const EdgeInsets.all(24), children: [
      Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(children: [
        TextField(onChanged: (v) => setState(() => search = v), decoration: const InputDecoration(labelText: 'ابحث برقم الغرفة أو النوع أو اسم الضيف', prefixIcon: Icon(Icons.search), suffixIcon: Icon(Icons.tune), border: OutlineInputBorder())),
      ...
      Row(children: [Text('${list.length} غرفة مطابقة', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), const Spacer(), Wrap(spacing: 8, children: ['available', 'occupied', 'cleaning', 'maintenance'].map((s) => Chip(avatar: Icon(Icons.circle, size: 11, color: statusColor(s)), label: Text('${statusLabel(s)} ${c.rooms.where((r) => r.status == s).length}'), side: BorderSide.none, backgroundColor: statusColor(s).withOpacity(.08))).toList())]),
      const SizedBox(height: 14),
