...
Future<void> showRoomDetails(BuildContext context, RoomRecord room) async {
  ...
  final stayContent = guest == null || stayBooking == null ? Row(children: [Icon(room.status == 'available' ? Icons.check_circle : Icons.info_outline, color: statusColor(room.status)), const SizedBox(width: 8), Text(room.status == 'available' ? 'الغرفة جاهزة لاستقبال حجز جديد' : 'لا توجد إقامة نشطة حالياً')]) : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Divider(height: 24), Text('الإقامة الحالية', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), DetailLine(icon: Icons.person_outline, label: 'الضيف', value: guest.name), DetailLine(icon: Icons.calendar_today_outlined, label: 'الفترة', value: '${dateLabel(stayBooking.checkIn)} — ${dateLabel(stayBooking.checkOut)}'), DetailLine(icon: Icons.receipt_long_outlined, label: 'رقم الحجز', value: stayBooking.number)]);
